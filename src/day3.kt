...
fun getWireCoords(wirePath: List<String>): List<Pair<Int, Int>> {
    ...
    for (instruction in wirePath) {
        ...
        for (step in 0 until distance) {
            coords.add(getNextCoordinate(coords.last(), direction))
        }
    }
...
fun getNextCoordinate(start: Pair<Int, Int>, direction: Char): Pair<Int, Int> {
    return when (direction) {
        'U' -> Pair(start.first, start.second + 1)
        'D' -> Pair(start.first, start.second - 1)
        'L' -> Pair(start.first - 1, start.second)
        'R' -> Pair(start.first + 1, start.second)
